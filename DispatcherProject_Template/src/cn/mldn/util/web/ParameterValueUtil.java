...
public class ParameterValueUtil {
	...
	public static boolean isBasic(String type) {
		return "int".equals(type) || "java.lang.Integer".equals(type) || "double".equals(type) || "long".equals(type)
				|| "java.lang.Long".equals(type) || "java.lang.Double".equals(type) || "java.lang.String".equals(type)
	 ...
	 *             产生的反射异常
	 */
	public static Object setActionMethodParameterValue(HttpServletRequest request, Object actionObject,
		...
		for (int x = 0; x < paramNames.length; x++) {
			try {//----------------------------------------------------------这里容易出错，所以加上try-----------catch:别让整个代码出现问题
				if(isBasic(types[x].getName())) {//判断是普通的类型
//					String value = request.getParameter(paramNames[x]);
					String value = ServletObjectUtil.getParam().getParameter(paramNames[x]);//----------------表单封装参数的接收
					values[x] = DataConverterUtil.converter(value, types[x].getName());
				}else if(isArray(types[x].getSimpleName())){//---------------------------------------------数组类型接收
//					String value[] = request.getParameterValues(paramNames[x]);//按照数组进行处理
					String value[] = ServletObjectUtil.getParam().getParameterValues(paramNames[x]);//按照数组进行处理----------------------表单封装参数的接收
					values[x] = DataConverterUtil.converterArray(value, types[x].getSimpleName());
				}else {//现在不是一个普通的类型，可能是vo类---------------------------------------------关键
					//获取vo的类型之后，就可以利用反射进行实例化了
	...
	private static String[] getActionParameterNames(Class<?> actionClass, Method actionMethod) throws Exception {
		...
		// 如果javassist开发包与Tomcat整合一起的话，那么就会出现Tomcat的CLASSPATH覆盖了Javassist设置的CLASSPATH------------------------关键注意
